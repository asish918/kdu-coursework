 ...
 */
@Component
public class VehicleService2 implements Factory {
     ...
     */
    @Autowired
    public VehicleService2(TyreService tyreService, SpeakerService speakerService, Inventory inventory) {
     ...
     * @return List of  {@link  Vehicle Vehicles}
    ...
    public List<Vehicle> generateVehicleList(int count) throws VehicleException, TyreException, SpeakerException {
        ...
        for (int i = 0; i < count; i++) {
            Tyre tyre = tyreService.generateTyre(TyreBrand.MRF);
            Speaker speaker = speakerService.generateSpeaker(SpeakerBrand.SONY);
